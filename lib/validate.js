...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '0fp2h', options ) );
	}
	if ( hasOwnProp( options, 'alpha' ) ) {
		opts.alpha = options.alpha;
		if ( !isNumber( opts.alpha ) || isnan( opts.alpha ) ) {
			return new TypeError( format( '0fp8h', 'alpha', opts.alpha ) );
		}
	...
	if ( hasOwnProp( options, 'alternative' ) ) {
		...
		if ( !isString( opts.alternative ) ) {
			return new TypeError( format( '0fp2i', 'alternative', opts.alternative ) );
		}
		if ( !contains( ALTERNATIVE, opts.alternative ) ) {
			return new Error( format( '0fp3t', 'alternative', ALTERNATIVE.join( '", "' ), opts.alternative ) );
		}
	}
	if ( hasOwnProp( options, 'mu' ) ) {
		opts.mu = options.mu;
		if ( !isNumber( opts.mu ) || isnan( opts.mu ) ) {
			return new TypeError( format( '0fp8h', 'mu', opts.mu ) );
		}
	}
	return null;
}
