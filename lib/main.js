...
function ttest( x ) {
	...
	if ( !isTypedArrayLike( x ) && !isNumberArray( x ) ) {
		throw new TypeError( format( '0fp8j', x ) );
	}
	len = x.length;
	if ( len < 2 ) {
		throw new Error( format( '0fpAn', x ) );
	}
	opts = {
		'mu': 0.0,
		'alpha': 0.05,
	...
	if ( arguments.length === 2 ) {
		if ( isObject( arguments[ 1 ] ) ) {
			...
			if ( !isTypedArrayLike( y ) && !isNumberArray( y ) ) {
				throw new TypeError( format( '0fpAj', y ) );
			}
		}
	} else if ( arguments.length > 2 ) {
		y = arguments[ 1 ];
		if ( !isTypedArrayLike( y ) && !isNumberArray( y ) ) {
			throw new TypeError( format( '0fpAa', y ) );
		}
